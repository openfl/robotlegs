...
class FuseInitializer extends BaseInitializer {
	...
	override public function addLayer(ViewClass:Class<Dynamic>, index:Int, total:Int, id:String):Void {
		...
		fuseConfig.transparent = true;
		fuseConfig.color = 0x00000000;

		var stage3DRenderContext:Stage3DRenderContext = cast renderContext;

		var fuse:Fuse = new Fuse(untyped ViewClass, fuseConfig, stage3DRenderContext.stage3D, Context3DRenderMode.AUTO, [stage3DRenderContext.profile]);
		...
		if (index == -1)
			layers.addLayer(placeHolderLayer);
		else
			layers.addLayerAt(placeHolderLayer, index);
